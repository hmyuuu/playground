...
# ╔═╡ e27ea374-7374-11ed-3bf4-05b441c66037
using QuanEstimation, Plots, PlutoUI, Random, LaTeXStrings

# ╔═╡ a581b75e-6cbd-4efe-89d5-7eb9094268f5
md"""
# QuanEstimation Tutorial
## Examples for Quantum Cramér–Rao Bounds
Let's first consider the single parameter estimation with a single qubit system. The Hamiltonian of the system is 

$$H=\frac{1}{2}\omega \sigma_3$$ 

where the frequency $\omega$ is the paramter to be estimated
"""

# ╔═╡ d595dd0c-5bae-49f7-96aa-a2e030ccfde8
begin
	# free Hamiltonian
	omega = 1.0
	sx = [0. 1.; 1. 0.0im]
...
# ╔═╡ 5968677f-55f6-4ffb-8dec-2e11572daf87
md"""
...
where $\sigma_{\pm}=\frac{1}{2}(\sigma_1 \pm \sigma_2)$ and $\gamma_{+}$, $\gamma_{-}$ are decay rates. 
"""

# ╔═╡ 39797d0c-afd5-4e1e-bc4a-9f8da2b1a794
begin
	# dissipation
	sp = [0. 1.; 0. 0.0im]
	sm = [0. 0.; 1. 0.0im]
...
# ╔═╡ b3e2f0bd-0cc3-4f3a-8aa6-347b4d285f94
md"""
The probe state is taken as $|+\rangle=\frac{1}{\sqrt{2}}(|0\rangle+|1\rangle)$. Here $|0\rangle$ and $|1\rangle$ are 
the eigenstates of $\sigma_3$ with the eigenvalues $1$ and $-1$, respectively.
"""

# ╔═╡ 3060713c-4a6b-4458-8845-e1eead1c0111
rho0 = 0.5*ones(2, 2)

# ╔═╡ d9413dfd-383a-4af0-bff4-35620accd95c
md"The measurement matrics for CFIM are taken as "

# ╔═╡ 3e605870-fc53-41a0-b042-a24638c1e0b6
begin
	# measurement
...
# ╔═╡ 4b0887ba-c93a-48df-906f-b5e82f3cea40
md"Now we can solve the dynamics to obtain the parametrized state, and then calculate the bounds"

# ╔═╡ c7f0c61b-7aeb-4c00-9068-8aa1f495ce0b
md"tmax: $(@bind tmax Slider(1:0.01:50, show_value=true, default=5) )"

# ╔═╡ f8fe15a2-14bf-4421-b99d-77453e4af172
md"tnum: $(@bind tnum Slider(10:10:1000, show_value=true, default=250) )"

# ╔═╡ 24f43a44-3ab8-49f0-8808-ff425cc1d594
tspan = range(0., tmax, length=tnum)

# ╔═╡ ad4d1f97-8c34-4fa6-8d3c-a7e5016ddf97
rho, drho = QuanEstimation.ode(tspan, rho0, H0, dH, decay)

# ╔═╡ 7fdbd9d6-fd48-450a-82a2-4c3b9caafbf5
begin
	I, F = Float64[], Float64[]
...
# ╔═╡ af797ea1-81d7-4e54-a3b4-e21101b793f5
plot(tspan[1:end-1], [I F], label=["CFI" "QFI"], xlabel="time",)

# ╔═╡ 0eb5c2f4-cf4a-4f96-be54-009f3ae82311
md"## Control optimization"

# ╔═╡ f21d354d-3679-42ba-9d95-785e60355060
# ╠═╡ skip_as_script = true
#=╠═╡
...
# ╔═╡ e48a8fee-f021-49b6-b3f3-ffc79ef4cff5
# ╠═╡ disabled = true
#=╠═╡
begin
	# control Hamiltonians 
	Hc = [sx, sy, sz]
	# initial guess for control coefficients
	cnum = length(tspan)-1
	...
	alg = QuanEstimation.autoGRAPE(Adam=true, max_episode=300, epsilon=0.01, beta1=0.90, beta2=0.99)
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "67a06f52d3d220ab259c4b7f8809616ccff689e9"
...
[[deps.QuanEstimation]]
deps = ["BoundaryValueDiffEq", "Convex", "DelimitedFiles", "Distributions", "Flux", "Interpolations", "IntervalSets", "LinearAlgebra", "OrdinaryDiffEq", "Printf", "QuadGK", "Random", "ReinforcementLearning", "SCS", "SparseArrays", "StableRNGs", "StatsBase", "Trapz", "Zygote"]
git-tree-sha1 = "f8292f665b8b763951309d28eba150ef5f238689"
